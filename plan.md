...
macOS版は身内配布用のzip作成まで確認済み。Step 1は完了扱いにする。
...
- UIはmacOS版で再現したDesktop UIを原則共有する
- macOS版とWindows版で、見た目、サイズ感、操作導線をできるだけ揃える
- Windows固有の常駐、表示、終了、タスクトレイ制御はプラットフォーム層へ分離する
- タスクトレイアイコンをクリックするとSplitLogの小型ウィンドウを表示する
- ウィンドウを閉じた場合は終了ではなく非表示にする
- 終了は設定/メニューから明示的に実行する
- Windowsでは旧macOS版 `sessions.json` の自動検知は行わない
- 手動で `sessions.json` を選択してインポートする機能はWindowsでも維持する
- 保存先はWindowsのユーザー別ローカル領域を使う
- グローバルショートカットは可能ならDesktop共通機能として対応するが、不安定な場合は後続対応に回す
...
- WindowsでSplitLogを起動できる
- タスクトレイ常駐として使える
- macOS版とほぼ同じDesktop UI/UXで主要機能を使える
- Windowsローカルにデータ保存できる
- 手動インポートが動作する
- zipで身内確認用に配布できる
